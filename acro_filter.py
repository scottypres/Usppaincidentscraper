...
# TIER 3: Possible acro — low altitude flying / flyby (risky behavior)
TIER3_PATTERNS = [
    # Low altitude / low flying
    r'\blow[\s-]+altitude\s+(?:fly|flight|pass|turn|circle|maneuver)',
    r'\bflying\s+low\b',
    r'\bfl(?:ying|ew)\s+(?:really\s+)?low[\s-]+level',
    r'\blow[\s-]+level\s+(?:circle|turn|flight|flying|pass)',
    r'\blow[\s-]+level\b.*\b(?:circle|turn|spiral|maneuvering)',
    r'\bdown\s+low\b',
    r'\bplaying\s+around\s+(?:down\s+)?low',
    r'\bmaneuvering\s+low\b',
    # Low pass / flyby
    r'\blow\s+(?:fly[\s-]*by|pass)\b',
    r'\bfly[\s-]*by\b.*\b(?:low|photo|video|camera|film)',
    r'\b(?:photo|video|camera|film).*\bfly[\s-]*by\b',
    r'\bfly[\s-]*by\s+activit',
    r'\bbuzzing\b',
]
...
def is_acro_incident(rec):
    ...
    # Tier 3: low altitude flying / flyby (risky behavior, possible acro)
    tier3_matches = find_matches(text, TIER3_PATTERNS)
    if tier3_matches:
        return True, 'possible', tier3_matches
